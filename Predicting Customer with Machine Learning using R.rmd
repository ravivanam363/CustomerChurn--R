...
1- no churn, 0- churn
 ...
 At 0.7 Cut-off, confusion matrix of Random forest model
 Reference
Prediction    0    1
         0  194   55
         1   41 1377
...
To Reduce the churn I will target the users by target based marketing which include email campaign, mail campaign, and offers.
From confMtrx3
1- no churn, 0- churn 
 At 0.7 Cut-off, confusion matrix of Random forest model
 Reference
Prediction    0    1
         ...
         1   41 1377
Assumptions:
1. cost per each customer 10$ (include email campaign, mail campaign, and offers)
2. Each time mail campaign is conducted, percentage of customers who are going to unsubscribe = 4%
losing a customer for lifetime = 500$
3 . Dollar $ benefit of a customer month = 80$
4. Number of months' customers stay after promotion = 5 months
5. Percentage of customers who respond positively = 20%


Calculations:
        Number of customers who are predicted to churn by the model = 249(194+55)
        Number of customers who will not churn among the predicted churn customers = 55
      not churning users from 20% assumption = .20*194 = 38
        Revenue from customers who responded positively= $80 *(38+55) = $7440 per month
        Cost of sending promotions to the churning customers = $10*194 = $1940
        Profit obtained to the company if the model is correct = $7440 - $1940 = $5500 per month
       If the model is incorrect the total revenue from the customers who responds positively to promotions = $80*55 = $4400 per month
        Profit obtained to the company if the model is incorrect = 4400-1940= $2460 per month
        Hence by using the results of the model at 0.7 cutoff the firm can earn a profit of 123% per month if the model prediction is correct and a loss of 23% per month if the model predicts incorrectly.
From the above two results we can conclude that 0.7 cutoff is a good threshold to choose for the random forest predictive model whose results will prove beneficial to the firm.
Revenue loss due to unsubscription = (0.04 * 194) * 500 = 3500
Profit for 5 month period:
For 5 months = 5 * 5500 - (Revenue loss due to unsubscription) = 27500 - 3500 = 24000$ for 5 months.
